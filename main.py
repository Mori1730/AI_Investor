...
def run_investment_analysis(stock_target: str) -> str:
    ...
    alpha_agent = Agent(
        role="策略獵人 Alpha",
        goal="整合即時股價與網路新聞，尋找台美股具備動能的進場點。",
        backstory="你擅長數據分析與新聞情緒解讀。你認為投資必須結合基本面數據與最新的市場消息。",
        tools=[fetch_stock_data, search_tool],
        llm=gemini_llm,
        verbose=True,
    )
    ...
    risk_agent = Agent(
        role="風控判官 Risk",
        goal="針對 Alpha 的提案進行壓力測試，找出潛在的利空因素與風險。",
        backstory="你是偏保守的風控專家。你的任務是質疑一切，確保投資組合不會暴露在不必要的風險中。",
        tools=[fetch_stock_data, search_tool],
        llm=gemini_llm,
        verbose=True,
    )
